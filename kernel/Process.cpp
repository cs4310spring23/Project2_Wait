...
Process::Process(ProcessID id, Address entry, bool privileged, const MemoryMap &map, u8 priority)
	: m_id(id), m_map(map), m_shares(id), m_priority(priority)
{
	m_state     	= Stopped;
	m_parent    	= 0;
	m_waitId    	= 0;
...
void Process::setPriorityLevel(PriorityLevel priorityLevel)
{
    if (priorityLevel >= MinPriority && priorityLevel <= MaxPriority)
    {
